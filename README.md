...
### 随机种子与图片复现
- **🔀 随机模式**（默认）：每次执行使用新随机种子，输出不同。执行后种子值自动更新到输入框并保存到 config。
- **⏸️ 固定模式**：相同种子输出相同结果。手动输入种子值后切换到此模式即可锁定。
- **图片复现**：ComfyUI 会将工作流（含种子值和 config）保存到 PNG 元数据中。拖放图片到 ComfyUI 还原工作流后，种子值和配置不变，重新执行即可得到相同的提示词输出。
- 种子值和模式（🔀/⏸️）会保存到预设中。
...
### 260721
1. 修复节点在 Nodes 2.0 中无限向下拉伸的 BUG。
...
2. 节点显示区域高度跟随节点大小变化（旧前端 + Nodes 2.0 双兼容）。
