...
| 元素 | 功能 |
|------|------|
| 🛎️ | 生成随机种子。点击后在种子输入框填入一个新随机种子，所有 txt 文件框的 🎲 词组预览随之刷新，模式自动切换为 ⏸️ 固定模式。确保种子值、按钮预览与后端实际输出三者完全一致，便于复现 |
...
| 🎲/📌 | 随机模式开关。🎲 绿色背景=随机模式已开启（点击固定当前选择）；📌 灰色背景=固定模式（点击开启随机选取一行）。📌 固定模式时词组选择按钮边框变红，提示关注手动选择的行；🎲 随机模式时词组按钮显示基于种子预计算的结果 |
| ⁉️ | 自定义间隔符。设置此 txt 输出与下一个 txt 输出之间的分隔符（最后一项的间隔符不生效） |
| × | 移除此 txt 文件 |
| txt 文件名 | 显示文件名 |
| 词组选择按钮 | 点击弹出选行窗口，选择要输出的那一行。支持搜索过滤。🎲 随机开启时按钮显示基于当前种子预计算的随机结果（与后端实际输出一致），随种子/启用状态/🎲 切换实时更新；🎲 随机关闭时显示手动选择的行 |
...
- 种子值和模式（🔀/⏸️）会保存到预设中。
- **词组按钮预览**：🎲 随机开启时，词组选择按钮显示基于当前种子预计算出的随机结果，与后端实际输出那一行完全一致。前端内置了与 CPython `random.Random` 字节级一致的 MT19937 实现（支持 0 ~ 2^53 种子），无需请求后端即可预计算。
