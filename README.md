...
| 🗳️ | 合并随机输出开关（绿色=开启）。开启后忽略常规拼接，从所有已启用 txt 的词组池中随机选取 N 个输出，用 `, ` 连接。开启时所有 txt 文件框边框变黄色。🗳️ 与数量输入框、▲▼ 微调按钮组合为一体 |
| 数量输入框 | 指定 🗳️ 合并随机输出的词组数量（最小 1）。右侧 ▲▼ 按钮可快速增减数值 |
| �️ | 生成随机种子。点击后在"种子输入框"填入一个新随机种子，每个 txt 文件框的 🎲 词组预览随之刷新，模式自动切换为"⏸️ 固定模式"。确保种子值、按钮预览与后端实际输出三者完全一致，便于复现 |
| �/⏸️ | 随机种子模式切换。🔀 随机模式（每次执行不同）；⏸️ 固定模式（相同种子输出相同结果，绿色背景）。用于图片复现 |
| 种子输入框 | 显示和输入种子值。随机模式下每次执行后自动更新；固定模式下保持不变 |
...
|------|------|
| ⋮⋮ | 拖拽手柄，拖动可调整输出顺序 |
| 🟢/🔴 | 切换此 txt 是否参与输出（🟢 绿色背景=开启；🔴 灰色背景=关闭） |
| 🎲/📌 | 随机模式开关。🎲 绿色背景=随机模式已开启（点击固定当前选择）；📌 灰色背景=固定模式（点击开启随机选取一行）。📌 固定模式时词组按钮边框变红，提示关注手动选择的行；🎲 随机模式时按钮显示基于种子预计算的结果 |
| ⁉️ | 自定义间隔符。设置此 txt 输出与下一个 txt 输出之间的分隔符（最后一项的间隔符不生效） |
| × | 移除此 txt 文件 |
| txt 文件名 | 显示文件名 |
| 词组选择按钮 | 点击弹出选行窗口，选择要输出的那一行。支持搜索过滤。🎲 开启时按钮显示基于当前种子预计算的随机结果（与后端实际输出一致），随种子/启用状态/🎲 切换实时更新；🎲 关闭时显示手动选择的行 |
...
- 词组选择窗口中默认显示译文（如有），但实际输出的是原 txt 文件中的原文。
- 窗口标题会标注"（译文显示/原文输出）"。
...
- 种子值和模式（🔀/⏸️）会保存到预设中。
- **词组按钮预览**：🎲 开启时，词组选择按钮显示基于当前种子预计算出的随机结果，与后端实际输出那一行完全一致。前端内置了与 CPython `random.Random` 字节级一致的 MT19937 实现（支持 0 ~ 2^53 种子），无需请求后端即可预计算。
...
7. txt 文件框内按钮图标优化。
    ...
    - 随机模式按钮：由固定 🎲 改为 🎲/📌 切换。🎲 绿色底=随机模式开启（点击固定当前选择）；📌 灰色底=固定模式（点击开启随机）。📌 固定模式时词组选择按钮边框变红，提示关注手动选择的行。
8. 工具栏第二行布局优化。
    - 🗳️ 与数量输入框、▲▼ 微调按钮组合为一体（无间隙），去除两侧 `|` 分隔线。
